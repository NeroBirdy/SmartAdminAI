...
async function saveUserState(params: SaveUserStateParams) {
    const { peerId, state, city, citiesList, page, organization, organizationsList } = params;
...
async function setUserAccessCode(peerId: number, accessCode: string, role: string) {
...
async function login(peerId: number, key: string) {
    try {
        const [client, employee] = await Promise.all([
            fakeApi.client.findUnique({ where: { accessCode: key }, select: { firstName: true, lastName: true } }),
            fakeApi.employee.findUnique({ where: { accessCode: key }, select: { firstName: true, lastName: true, role: true } }),
        ]);
...
async function chooseCity(peerId: number, cityList: string[]) {
    if (cityList.length === 0) {
        ...
        await saveUserState({ peerId: peerId, state: 'choose_organization', city: cityList[0] });
        await sendChooseOrganizationKeyboard(peerId);
    } else {
        const keyboard = buildKeyboard(cityList, 1, 'choose_city');
