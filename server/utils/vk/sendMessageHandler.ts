...
async function sendStartMessage(peerId: number) {
    const keyboard = Keyboard.builder()
        ...
        .row()
        .textButton({ label: 'Войти в систему', color: Keyboard.PRIMARY_COLOR, payload: { cmd: 'login' } })
        .oneTime();
...
async function sendChooseOrganizationKeyboard(peerId: number) {
    ...
    const keyboard = buildKeyboard(organizationsList, 1, 'choose_organization');
    await vk.api.messages.send(
        {
            peer_id: peerId,
...
async function sendCompleteLoginMessage(peerId: number, firstName: string, lastName: string) {
    await vk.api.messages.send(
        {
            peer_id: peerId,
...
async function sendHelloMessage(peerId: number) {
    ...
    switch (role) {
        ...
        case 'INSTRUCTOR':
            ...
            keyboard.textButton({ label: 'Управление расписание', color: Keyboard.PRIMARY_COLOR, payload: { cmd: 'scheduleManagement' } }).row()
                .textButton({ label: 'Моё расписание', color: Keyboard.PRIMARY_COLOR, payload: { cmd: 'checkSchedule' } }).row();
            break;
    ...
    keyboard.textButton({ label: 'Выйти из системы', color: Keyboard.POSITIVE_COLOR, payload: { cmd: 'logout' } });
...
async function sendScheduleMenagementMessage(peerId: number, keyboard: Keyboard, text: string) {
    return await vk.api.messages.send({
        peer_id: peerId,
        message: text,
        keyboard: keyboard,
...
async function sendCancellationLessonMessage(peerId: number, keyboard: Keyboard) {
