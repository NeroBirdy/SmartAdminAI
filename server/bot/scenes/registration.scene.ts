...
export const registrationScene = new StepScene("registration", [
  ...
  async (context) => {
    ...
    if (context.scene.step.firstTime) {
      const keyboard = Keyboard.builder()
        ...
        .row()
        .textButton({
          label: "Указать другой пол",
          payload: { cmd: "changeGender" },
        })
        .row()
        .textButton({
          label: `${context.scene.state.child ? "Поменять имя ребенка" : "Поменять имя"}`,
          payload: { cmd: "changeName" },
        })
        .row()
        .textButton({
          label: `${context.scene.state.child ? "Поменять фамилию ребенка" : "Поменять фамилию"}`,
          payload: { cmd: "changeSurname" },
        })
        .row()
        .textButton({
          label: `${context.scene.state.child ? "Поменять дату рождения ребенка" : "Поменять дату рождения"}`,
          payload: { cmd: "changeBirthDate" },
        })
        .row()
        .textButton({
          label: "Поменять номер телефона",
          payload: { cmd: "changePhone" },
        })
        .row()
        .textButton({
          label: "Поменять почту",
          payload: { cmd: "changeEmail" },
        })
        .row()
        .oneTime();
      ...
      if (context.scene.state.child) {
        keyboard
          .textButton({
            label: "Поменять имя родителя",
            payload: { cmd: "changeParentName" },
          })
          .row();
        keyboard
          .textButton({
            label: "Поменять фамилию родителя",
            payload: { cmd: "changeParentSurname" },
          })
          .row()
          .oneTime();
      }
  ...
  async (context) => {
    ...
    if (context.scene.step.firstTime) {
      ...
      if (isChild) {
        registration = await createNewUser(
          isChild,
          gender,
          name,
          surname,
